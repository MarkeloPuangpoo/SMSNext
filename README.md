...
- **🎓 จัดการนักเรียน (Students)**: ดูรายชื่อ ค้นหา และจัดการข้อมูลนักเรียน (รองรับข้อมูลละเอียด เช่น น้ำหนัก ส่วนสูง ข้อมูลครอบครัว)
...
- **📥 นำเข้าข้อมูล (Bulk Import)**: รองรับการนำเข้าข้อมูลนักเรียนจำนวนมากผ่านไฟล์ CSV
- **� API Integration**: เชื่อมต่อข้อมูลนักเรียนกับระบบภายนอกผ่าน API ที่ปลอดภัย
...
## 📥 การนำเข้าข้อมูลนักเรียน (CSV Import)

ระบบรองรับการนำเข้าข้อมูลนักเรียนทีละหลายคนผ่านไฟล์ CSV โดยรองรับคอลัมน์ดังนี้:
...
*หมายเหตุ: วันที่เกิด (`birth_date`) รองรับรูปแบบ `DD/MM/YYYY` (เช่น 01/06/2008) และ `YYYY-MM-DD`*

## 🔌 API สำหรับเชื่อมต่อภายนอก

ระบบมี API Endpoint สำหรับตรวจสอบข้อมูลนักเรียนจากระบบอื่น:

**Endpoint**: `GET /api/students/check`

**Parameters**:
- `student_number`: รหัสนักเรียนที่ต้องการค้นหา

**Headers**:
- `x-api-key`: รหัสลับสำหรับยืนยันตัวตน (ต้องตรงกับ `API_SECRET_KEY` ใน .env)

**Example Request**:
```http
GET /api/students/check?student_number=12345 HTTP/1.1
...
3. **ตั้งค่า Environment Variables**
   สร้างไฟล์ `.env.local` และใส่ค่า Config ของ Supabase และ API Key:
   ```env
